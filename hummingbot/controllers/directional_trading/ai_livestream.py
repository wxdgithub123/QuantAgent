...
class AILivestreamController(DirectionalTradingControllerBase):
    def __init__(self, config: AILivestreamControllerConfig, *args, **kwargs):
        ...
        self._ml_signal_listener_initialized = False
    ...
    def _init_ml_signal_listener(self):
        ...
        try:
            ...
            self._ml_signal_listener_initialized = True
            self.logger().info(f"ML signal listener initialized on topic: {topic}")
        except Exception as e:
            self.logger().warning(f"ML signal listener not yet ready: {str(e)}")
            self._ml_signal_listener = None

    def _handle_ml_signal(self, signal: dict, topic: str):
        """Handle incoming ML signal"""
        self.logger().info(f"ML signal received on {topic}: {signal}")
    ...
    async def update_processed_data(self):
        if not self._ml_signal_listener_initialized:
            self._init_ml_signal_listener()

    async def on_start(self):
        """Retry ML signal listener initialization until MQTT Gateway is ready."""
        retries = 0
        while not self._ml_signal_listener_initialized and retries < 30:
            await asyncio.sleep(2)
            retries += 1
            self._init_ml_signal_listener()
        if not self._ml_signal_listener_initialized:
            self.logger().error("Failed to initialize ML signal listener after 30 retries")

    async def control_task(self):
        """Override to work even when market data is not ready (for demo purposes)."""
        ...
        if self.executors_update_event.is_set():
            ...
            if len(executor_actions) > 0:
                self.logger().info(f"Creating executor actions: {executor_actions}")
